...
func iterate(state *state.State) *flaw.Error {
	logger.Debug("rakuten-iterate", state)
	...
	for _, transaction := range res.Transactions {
		logger.Debug("rakuten-iterate-transaction", transaction)
		...
		if flawError != nil {
			logger.Warn("rakuten-iterate-transaction-error", flawError.Wrap("cannot iterate"))
